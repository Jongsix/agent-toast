...
- **원격 알림** - 원격 Linux 서버의 Claude Code 알림을 SSH 터널로 수신 (설정 → Remote 탭)
- **알림 스타일** - 알림 배경 불투명도, 배경색, 텍스트 색상을 자유롭게 커스터마이징
- **다국어 지원** - 한국어/영어 UI
...
- SSH 역방향 터널로 원격 서버의 알림을 로컬 HTTP 서버(127.0.0.1:9876)로 전달
